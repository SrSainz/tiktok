...
PLANNER_SLOTS = [
    {"slot_key": "morning", "label": "Mañana", "publish_time": "09:30", "strategy": "Clip claro y facil de consumir para la mañana."},
    {"slot_key": "lunch", "label": "Comida", "publish_time": "13:30", "strategy": "Hook rapido para el primer pico del dia."},
    {"slot_key": "afternoon", "label": "Tarde", "publish_time": "18:30", "strategy": "Momento con energia para la tarde."},
    {"slot_key": "prime", "label": "Prime", "publish_time": "21:30", "strategy": "La apuesta mas fuerte del dia."},
    {"slot_key": "late", "label": "Noche", "publish_time": "23:00", "strategy": "Ultimo clip mas comentable del dia."},
]
...
def build_daily_post_plan(
    *,
    ...
    candidates = discover_creator_videos(
        ...
        max_results=max(max_results, posts_per_day + reserve_count + 6),
        mode=mode,
        log_fn=log_fn,
    )
    ...
    for slot in PLANNER_SLOTS[: max(1, min(posts_per_day, len(PLANNER_SLOTS)))]:
        chosen = None
        for candidate in candidates:
            channel_key = str(candidate.channel or "").strip().lower()
            if channel_key and channel_key in used_channels:
                continue
            chosen = candidate
            break
        if not chosen:
            continue
        used_channels.add(str(chosen.channel or "").strip().lower())
        alternatives = []
        for candidate in candidates:
            if candidate.url == chosen.url:
                continue
            channel_key = str(candidate.channel or "").strip().lower()
            if channel_key and channel_key in used_channels:
                continue
            ...
            if len(alternatives) >= max(0, slot_option_count - 1):
                break
        slots.append(
            {
        ...
        candidates = [candidate for candidate in candidates if candidate.url != chosen.url]
    reserves = [
        {
            "title": candidate.title,
            "url": candidate.url,
        ...
        for candidate in candidates[: max(0, reserve_count)]
    ]
    return {
